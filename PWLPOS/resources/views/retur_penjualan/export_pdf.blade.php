...
<body>
    ...
    <h3 class="text-center">LAPORAN DATA RETUR PENJUALAN</h3>
        ...
        <tbody>
            @foreach($retur as $r)
                <tr>
                    ...
                    <td>{{ $r->penjualan->penjualan_kode ?? '-' }}</td>
                    <td>{{ $r->barang->nama_barang ?? '-' }}</td>
                    <td class="text-center">{{ $r->jumlah }}</td>
                    <td>{{ $r->alasan }}</td>
                    <td class="text-center">{{ \Carbon\Carbon::parse($r->tanggal_retur)->format('d/m/Y') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
